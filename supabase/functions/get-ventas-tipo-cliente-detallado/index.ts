...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { mes, aliado, sucursal, marca, rubro, portafolio_interno, consumo_masivo, version, presentacion, region, estado, tipo_cliente } = body;
    // Función para convertir valores de filtro
    const convertirFiltro = (valor)=>{
    ...
    const { data, error } = await supabaseClient.rpc('get_ventas_region_tipo_cliente_detallado', {
      ...
      p_presentacion: convertirFiltro(presentacion),
      p_region: convertirFiltro(region),
      p_estado: convertirFiltro(estado),
