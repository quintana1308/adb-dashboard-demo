-- Forzar recreación completa de la función get_activacion_vendedor
-- Eliminar todas las versiones existentes y crear una nueva con tipos correctos
...
-- Crear función completamente nueva con tipos correctos
...
BEGIN
  ...
  SELECT 
    ...
    -- Convertir TEXT a BIGINT para semana1 y semana6, manejar valores no numéricos
    COALESCE(
      CASE 
...
COMMENT ON FUNCTION get_activacion_vendedor IS 'Función para obtener datos de activación por vendedor con tipos corregidos - S1 y S6 convertidos de TEXT a BIGINT';

-- Refrescar esquema para asegurar que los cambios se apliquen
