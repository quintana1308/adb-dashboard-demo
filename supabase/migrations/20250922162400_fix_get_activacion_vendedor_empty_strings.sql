-- Corrección final para manejar cadenas vacías en get_activacion_vendedor
-- El problema es que hay campos con "" que no se pueden convertir a bigint
...
BEGIN
  ...
  SELECT 
    ...
    -- Manejo seguro de conversión TEXT a BIGINT para S1 y S6
    CASE 
      WHEN av."CARTERA_SEMANA1" IS NULL OR TRIM(av."CARTERA_SEMANA1") = '' OR TRIM(av."CARTERA_SEMANA1") = '0' 
      THEN 0
      WHEN TRIM(av."CARTERA_SEMANA1") ~ '^[0-9]+$' 
      THEN TRIM(av."CARTERA_SEMANA1")::bigint
      ELSE 0
    ...
    CASE 
      WHEN av."CARTERA_SEMANA6" IS NULL OR TRIM(av."CARTERA_SEMANA6") = '' OR TRIM(av."CARTERA_SEMANA6") = '0' 
      THEN 0
      WHEN TRIM(av."CARTERA_SEMANA6") ~ '^[0-9]+$' 
      THEN TRIM(av."CARTERA_SEMANA6")::bigint
      ELSE 0
    END as cartera_semana6,
    CASE 
      WHEN av."ACTIVACION_SEMANA1" IS NULL OR TRIM(av."ACTIVACION_SEMANA1") = '' OR TRIM(av."ACTIVACION_SEMANA1") = '0' 
      THEN 0
      WHEN TRIM(av."ACTIVACION_SEMANA1") ~ '^[0-9]+$' 
      THEN TRIM(av."ACTIVACION_SEMANA1")::bigint
      ELSE 0
    ...
    CASE 
      WHEN av."ACTIVACION_SEMANA6" IS NULL OR TRIM(av."ACTIVACION_SEMANA6") = '' OR TRIM(av."ACTIVACION_SEMANA6") = '0' 
      THEN 0
      WHEN TRIM(av."ACTIVACION_SEMANA6") ~ '^[0-9]+$' 
      THEN TRIM(av."ACTIVACION_SEMANA6")::bigint
      ELSE 0
...
COMMENT ON FUNCTION get_activacion_vendedor IS 'Función para obtener datos de activación por vendedor - manejo seguro de cadenas vacías';
