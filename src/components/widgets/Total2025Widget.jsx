...
const Total2025Widget = ({ filtros = {}, forceRefresh = 0 }) => {
  ...
  const fetchTotal2025 = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('get_ventas_total_2025', params)
      
      if (error) {
        console.error('Error en RPC get_ventas_total_2025:', error)
        throw error
      }
      
      const resultado = parseFloat(data) || 0
      
      setTotal2025(resultado)
    } catch (error) {
      console.error('Error fetching total 2025:', error)
      setTotal2025(0)
    } finally {
      setLoading(false)
    }
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          <p className="text-white/80 text-xs font-medium uppercase tracking-wide">TOTAL 2025</p>
          <p className="text-2xl font-bold text-white mt-1">
            {total2025?.toLocaleString('es-ES', { 
...
export default Total2025Widget
