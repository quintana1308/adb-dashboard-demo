...
const VentasGrupoWidget = ({ filtros, forceRefresh, onFiltroInternoChange }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { data: functionData, error: functionError } = await supabase.functions.invoke('get-ventas-grupo', {
      ...
      console.error('Error al cargar datos del widget VentasGrupo:', error)
  ...
  const handleBarClick = (data, index) => {
    if (data && data.GRUPO && onFiltroInternoChange) {
      // Aplicar filtro por grupo específico
      onFiltroInternoChange('portafolio_interno', data.GRUPO)
    }
  }
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
          ...
          <p className="text-xs text-gray-500 mt-1 italic">Haz clic para filtrar por este grupo</p>
        </div>
      )
    }
    return null
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-6">
      ...
      <div className="flex justify-end items-center mb-2 h-6 gap-2">
        ...
        <ExpandWidgetButton title="Ventas por Grupo">
        ...
        <ExcelExportButton
          ...
          filename="ventas_grupo"
          title="Reporte de Ventas por Grupo"
          sheetName="Ventas Grupo"
          columns={[
...
export default VentasGrupoWidget
