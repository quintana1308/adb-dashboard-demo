...
const VentasTipoClienteBarrasWidget = ({ filtros, forceRefresh, onFiltroInternoChange }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { data: functionData, error: functionError } = await supabase.functions.invoke('get-ventas-tipo-cliente', {
      ...
      console.error('Error al cargar datos del widget VentasTipoClienteBarras:', error)
  ...
  const handleBarClick = (data, index) => {
    if (data && data.TIPOCLIENTE && onFiltroInternoChange) {
      // Aplicar filtro por tipo cliente específico
      onFiltroInternoChange('tipo_cliente', data.TIPOCLIENTE)
    }
  }
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
          ...
          <p className="text-xs text-gray-500 mt-1 italic">Haz clic para filtrar por este tipo de cliente</p>
        </div>
      )
    }
    return null
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-6">
      ...
      <div className="flex justify-end items-center mb-2 h-6 gap-2">
        ...
        <ExpandWidgetButton title="Ventas por Tipo de Cliente (Barras)">
        ...
        <ExcelExportButton
          ...
          filename="ventas_tipo_cliente_barras"
          title="Reporte de Ventas por Tipo de Cliente (Barras)"
...
export default VentasTipoClienteBarrasWidget
