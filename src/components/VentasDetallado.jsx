...
import VentasGrupoMesWidget from './widgets/VentasGrupoMesWidget';
import VentasGrupoAnioWidget from './widgets/VentasGrupoAnioWidget';
import MapaVenezuelaWidget from './widgets/MapaVenezuelaWidget';
import VentasMesesWidget from './widgets/VentasMesesWidget'

const VentasDetallado = () => {
  ...
  // Función para manejar clic en grupo
  const handleGrupoClick = (grupo) => {
    setFiltrosSeleccionados(prev => ({
      ...prev,
      portafolio_interno: grupo
    }))
  }

  // Función para manejar clic en mes
  ...
  // Función para manejar filtros internos (para widgets clickeables)
  const handleFiltroInternoChange = (tipoFiltro, valor) => {
    setFiltrosSeleccionados(prev => ({
      ...prev,
      [tipoFiltro]: valor
    }))
  ...
  const actualizarFiltrosDependientes = async (tipo, valor, filtrosActuales) => {
    try {
      ...
      // Filtros de producto: rubro -> portafolio_interno -> consumo_masivo
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      {/* Header */}
      <DashboardHeader title="Ventas Detallado">
        <VentasNavMenu currentPage="ventas" />
      </DashboardHeader>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:flex md:flex-wrap gap-3 sm:gap-4 md:gap-3 mt-3 md:mt-0">
              ...
              <div className="flex items-center space-x-2 min-w-0">
                ...
                <select 
                  ...
                  className="border border-gray-200 rounded px-2 sm:px-3 py-1 text-sm bg-white text-gray-700 focus:border-gray-300 focus:outline-none min-w-0 flex-1 sm:flex-none sm:w-20 md:w-28"
                  disabled={loading}
                >
      ...
      <div className="p-4 sm:p-6 flex-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-20 gap-6">
          {/* Columna Izquierda - 65% (13/20) - Mismo ancho que widget de tabla */}
          <div className="lg:col-span-13">
            <VentasGrupoAnioWidget 
              filtros={filtrosSeleccionados}
              forceRefresh={forceRefresh}
...
export default VentasDetallado
